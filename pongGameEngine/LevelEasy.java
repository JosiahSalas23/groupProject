...
// when the state of the game is (pong) and the user selects the single player option/easy.
...
public class LevelEasy {
	...
	public void checkCollisionLong(PaddleLong paddle) {
		...
		// checks to see if there is no space between the ball and the paddle using AABB (Allined-Axis-Bounding-Box) algorithm technique
		if (paddle.position.x < ball.position.x + ball.WIDTH &&
				paddle.position.x + paddle.WIDTH > ball.position.x &&
				paddle.position.y < ball.position.y + ball.HEIGHT &&
				paddle.position.y + paddle.HEIGHT > ball.position.y) {
	...
	public void update(Window window) {
		...
		if (!pause) {
			...
			if(player1Bomb && !bombReleased) {
				...
				// release the bomb if key is pressed
			...
			// checks for collision if a power up dropped
