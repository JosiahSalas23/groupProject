...
public class LevelEasy extends LevelDif {
	...
	public void update(Window window) {
		...
		// if a powerball hasn't already spawned and there has been five ball hits since the last power up spawn a power up
		...
		if (super.player1LongPaddle) {
			...
			if (super.ball.movement.x < 0)
				checkCollisionLong(super.player1Long);
		...
		if (super.player2LongPaddle) {
			...
			if (super.ball.movement.x > 0)
				checkCollisionLong(super.player2Long);
			
		} // end if
		
		// if player 1 and player 2 don't have a modified paddle powerup check normal paddle collision
		...
		if (super.ball.movement.y > 0 && super.ball.position.y > super.player2.position.y + 0.125f && super.ball.movement.x > 0 && super.ball.position.x > -0.55f) {
			...
			super.player2.moveAIUpEasy();
			
		} // end if
		
		if (super.ball.movement.y < 0 && super.ball.position.y < super.player2.position.y + 0.125f && super.ball.movement.x > 0 && super.ball.position.x > -0.55f) {
			
			super.player2.moveAIDownEasy();
		...
		if (super.ball.movement.y > 0 && super.ball.position.y > super.player2Long.position.y + 0.25f && super.ball.movement.x > 0 && super.ball.position.x > -0.55f) {
			...
			super.player2Long.moveAIUpEasy();
			
		} // end if
		
		if (super.ball.movement.y < 0 && super.ball.position.y < super.player2Long.position.y + 0.25f && super.ball.movement.x > 0 && super.ball.position.x > -0.55f) {
			
			super.player2Long.moveAIDownEasy();
