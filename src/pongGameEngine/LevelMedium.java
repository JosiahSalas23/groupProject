...
// when the state of the game is (pong) and the user selects the single player option/medium.
...
public class LevelMedium {
	...
	public void checkCollision(Paddle paddle) {
		...
		// checks to see if there is no space between the ball and the paddle using AABB (Allined-Axis-Bounding-Box) collison algorithm
		if (paddle.position.x < ball.position.x + ball.WIDTH &&
			...
			if ((player1SlowPaddle || player1InvisPaddle)  && ball.movement.x < 0)
				PowerHitCount++;
			
			if ((player2SlowPaddle || player2InvisPaddle) && ball.movement.x > 0)
				PowerHitCount++;
	...
	// this function will be called once a paddle makes contact with a ball. It moves the ball in the opposite direction
	...
	public void giveAdvantageToPlayer1() {
		...
		int selectPowerUp;
		selectPowerUp = (int)(Math.random() * 5 + 1);
	...
	public void update(Window window) {
		...
		if (!pause) {
			...
			if(player2Bomb && !bombReleased) {
				...
				if (bomb.position.y > player1.position.y && bomb.position.y < player1.position.y + player1.HEIGHT) {
			...
			// AI logic for normal paddle
			if (ball.movement.y > 0 && ball.position.y > player2.position.y + 0.125f && ball.movement.x > 0 && ball.position.x > -0.55f) {
				
				// if player 2 is slowed down move them slowly 
				if (player2SlowPaddle)
					player2.moveAIUpSlow();
				
				// if not move them normally
				else
					player2.moveAIUpMedium();
			...
			if (ball.movement.y < 0 && ball.position.y < player2.position.y + 0.125f && ball.movement.x > 0 && ball.position.x > -0.55f) {
				...
				// if player 2 is slowed down move them slowly 
				if (player2SlowPaddle)
					player2.moveAIDownSlow();
				
				// if not move them normally
				else
					player2.moveAIDownMedium();
				
			} // end if
			
			// long paddle update for ai
			if (ball.movement.y > 0 && ball.position.y > player2Long.position.y + 0.25f && ball.movement.x > 0 && ball.position.x > -0.55f) {
				
				// if player 2 is slowed down move them slowly 
				if (player2SlowPaddle)
					player2Long.moveAIUpSlow();
				
				// if not move them normally
				else
					player2Long.moveAIUpMedium();
			...
			if (ball.movement.y < 0 && ball.position.y < player2Long.position.y + 0.25f && ball.movement.x > 0 && ball.position.x > -0.55f) {
				...
				// if player 2 is slowed down move them slowly 
				if (player2SlowPaddle)
					player2Long.moveAIDownSlow();
				
				// if not move them normally
				else
					player2Long.moveAIDownMedium();
				
			} // end if
			
			// small paddle update for ai
			if (ball.movement.y > 0 && ball.position.y > player2Short.position.y + 0.075f && ball.movement.x > 0 && ball.position.x > -0.55f) {
				
				// if player 2 is slowed down move them slowly 
				if (player2SlowPaddle)
					player2Short.moveAIUpSlow();
				
				// if not move them normally
				else
					player2Short.moveAIUpMedium();
			...
			if (ball.movement.y < 0 && ball.position.y < player2Short.position.y + 0.075f && ball.movement.x > 0 && ball.position.x > -0.55f) {
				...
				// if player 2 is slowed down move them slowly 
				if (player2SlowPaddle)
					player2Short.moveAIDownSlow();
				
				// if not move them normally
				else
					player2Short.moveAIDownMedium();
	...
	public void draw() {
		...
		if(!player1LongPaddle && !player2LongPaddle && !player1ShortPaddle && !player2ShortPaddle && !player1SlowPaddle 
				&& !player2SlowPaddle && !player1InvisPaddle && !player2InvisPaddle && !player1DestroyedPaddle 
				&& !player2DestroyedPaddle) {
